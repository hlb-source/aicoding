...
1. **设备插件优先级启动**：
   ...
   - 使用 `init containers` 等待设备就绪后才启动主插件

2. **设备插件启动流程**：
   - 设备插件启动后立即扫描系统中的 RDMA 设备
   - 基于设备唯一标识（PCIe 地址 + 序列号 + GUID）识别设备
   - 检测设备路径变化，生成新的 CDI 配置文件到 `/var/run/cdi` 目录
   - 使用原子操作更新 CDI 配置（先写临时文件，再重命名）

3. **健康检查机制**：
   - 设备插件实现 `readinessProbe`，检查 CDI 配置是否已更新
   - 只有当 CDI 配置更新完成后，设备插件才标记为就绪
   - 容器的 `affinity` 规则确保只调度到设备插件就绪的节点

4. **容器启动验证**：
   - 容器启动前通过 `preStart` 钩子验证设备路径存在性
   - 实现退避重试机制，确保 CDI 配置已生效
   - 验证设备路径与 CDI 配置中的路径匹配

5. **实时监控**：
   - 设备插件监控设备状态变化，实时更新 CDI 配置
   - 当检测到设备路径变化时，立即更新 CDI 配置
   - 实现事件通知机制，通知相关容器配置已更新
...
3. **启动健康检查**：
   - 设备插件实现 `readinessProbe` 和 `livenessProbe`
   - 确保设备插件完全初始化并更新 CDI 配置后才标记为就绪
   - 容器的 `startupProbe` 检查设备插件状态

4. **延迟容器启动**：
   - 设备插件启动后立即扫描设备并更新 CDI 配置
   - 容器启动前通过 `preStart` 钩子验证 CDI 配置有效性
   - 实现退避重试机制，确保 CDI 配置已更新

5. **监控和恢复**：
   - 设备插件监控设备状态变化，实时更新 CDI 配置
   - 容器启动失败时自动重试，等待 CDI 配置更新
   - 实现事件通知机制，设备变化时通知相关容器
